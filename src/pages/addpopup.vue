    ...
    >
      <v-card>
        <v-toolbar color="orange">
          ...
          <v-toolbar-title
            >점령전 {{ type == 1 ? "방덱" : "공덱" }} 추가</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-toolbar-items>
        ...
        <v-tabs id="elemental" bg-color="deep-purple-darken-4" center-active>
          <v-tab @click="tabChange('Fire_a')"
            ><img :src="require(`@/assets/images/Fire_Icon.png`)"
          /></v-tab>
          <v-tab @click="tabChange('Water_a')"
            ><img :src="require(`@/assets/images/Water_Icon.png`)"
          /></v-tab>
          <v-tab @click="tabChange('Wind_a')"
            ><img :src="require(`@/assets/images/Wind_Icon.png`)"
          /></v-tab>
          <v-tab @click="tabChange('Light_a')"
            ><img :src="require(`@/assets/images/Light_Icon.png`)"
          /></v-tab>
          <v-tab @click="tabChange('Dark_a')"
            ><img :src="require(`@/assets/images/Dark_Icon.png`)"
          /></v-tab>
        </v-tabs>
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      await this.$axios
        .post("/api/v1/summonerswar/enemyTeam-save", saveData)
        .then((res) => {
          if (res.data == "SUCCESS") {
...
#addMonsterList .v-list-item > div {
  width: calc(25% - 5px);
  position: relative;
  text-align: center;
}
#addMonsterList .v-list-item > div.active:after {
  content: "";
  position: absolute;
  top: 10px;
  left: 10px;
...
#addMonsterList .v-list-item img {
  display: block;
  width: 100%;
}
</style>