<template>
  <v-dialog v-model="dialog" transition="dialog-bottom-transition" fullscreen>
    <v-card>
      <v-toolbar>
        <v-icon @click="dialog = false" style="margin-right: 16px">mdi-arrow-left</v-icon>
      ...
      <v-list lines="two" id="occupation">
        <div class="contentBox">
          ...
          <v-list-item>
            <div :class="selectedMonsterList.length > 0 ? '' : 'leader'">
              <img v-if="selectedMonsterList[0]" :src="require(`@/assets${selectedMonsterList[0].image_url}`)"
                @click="monsterSelected(selectedMonsterList[0])" />
              <img v-if="!selectedMonsterList[0]" :src="require(`@/assets/images/unit_select_icon.png`)" />
            </div>
            <div>
              <img v-if="selectedMonsterList[1]" :src="require(`@/assets${selectedMonsterList[1].image_url}`)"
                @click="monsterSelected(selectedMonsterList[1])" />
              <img v-if="!selectedMonsterList[1]" :src="require(`@/assets/images/unit_select_icon.png`)" />
            </div>
            <div>
              <img v-if="selectedMonsterList[2]" :src="require(`@/assets${selectedMonsterList[2].image_url}`)"
        ...
        <div class="contentBox">
          ...
          <v-list-item subtitle="Notify me about updates to apps or games that I downloaded" title="Notifications"
