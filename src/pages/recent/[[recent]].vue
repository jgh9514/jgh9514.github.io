        ...
        <v-col cols="9">
          <v-card color="blue-grey-darken-1" class="mx-auto" max-width="420">
            <v-autocomplete
              ...
              multiple
              hide-details
              v-model:search-input.sync="searchText"
              @update:model-value="searchText = ''"
              placeholder="몬스터 선택"
              :custom-filter="
                (itemTitle, queryText, item) => {
                  const textOne = item.raw?.kr_name?.toLowerCase() || '';
                  const textTwo = item.raw?.un_name?.toLowerCase() || '';
                  const textThree = item.raw?.modified_kr_name?.toLowerCase() || '';
...
const search = async (isFirst = false) => {
  ...
  const searchData = $gfn_searchDataExtraction(schData.value);
  const res = await $api.get("/summonerswar/recent/enemyTeam-list", searchData);
  monsterList.value = [...res];

  totalPage.value = await $api.get("/summonerswar/recent/total-page-count", searchData);
};
