...
const init = async () => {
  if ($gfn_isEmpty(startDate.value) && $gfn_isEmpty(endDate.value) && infoData.value.init_data_yn === 'Y') {
...
const updateDateRange = async (newVal) => {
  if (newVal.length === 1) {
    const date = $gfn_convertDateToStringYYYYMMDD(new Date(newVal[0]), '-')
    startDate.value = date
    endDate.value = date
    dateText.value = `${date} ~ ${date}`
  } else if (newVal.length > 1) {
    const start = $gfn_convertDateToStringYYYYMMDD(new Date(newVal[0]), '-')
    const end = $gfn_convertDateToStringYYYYMMDD(new Date(newVal[newVal.length - 1]), '-')
